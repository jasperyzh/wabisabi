<template>
  <main>
    <pre class="marked">
# d3-start  
[Draw line chart by using d3 library in VueJS (Corona virus live chart)](https://www.youtube.com/watch?v=Sc7wl8_O0wk)
try out svg/html manipulation with d3js 
- [learn D3](https://observablehq.com/@d3/learn-d3)
- try https://github.com/fiduswriter/Simple-DataTables data from government 
...
- [Data.gov.my](https://www.data.gov.my/)
    ...
    <div class="line-chart-container">
      ...
      <svg
        class="line-chart"
        :viewBox="`0 0 ${chartWidth + marginLeft + marginRight} ${
...
// import "https://cdnjs.cloudflare.com/ajax/libs/numeral.js/2.0.6/numeral.min.js";
...
export default {
  ...
  created() {
    ...
    this.getCountryTimeline("US").then((res) => {
  ...
  methods: {
    async getCountryTimeline(country) {
      return await fetch(`https://corona-api.com/countries/${country}`);
    ...
    getGridXAxis() {
      return this.getXAxis().tickSize(this.chartHeight).tickFormat("");
    },
    getGridYAxis() {
      return this.getYAxis().tickSize(-this.chartWidth).tickFormat("");
    },
  },
  directives: {
    axis(el, binding) {
